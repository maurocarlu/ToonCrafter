...
sys.path.insert(0, str(Path("./scriptPanelManga").resolve()))
try:
            ...
            if len(generated_segments) == len(frame_pairs):
                ...
                if use_rife:
                    with st.spinner("Aumentando la fluidità con RIFE..."):
                        ...
                        cmd_rife = ["python", "./ECCV2022-RIFE/inference_video.py", "--exp", "1", "--video", str(final_video_path), "--output", str(rife_out), "--fp16", "--fps", str(final_fps)]
                        result_rife = subprocess.run(cmd_rife, capture_output=True, text=True, cwd="./ECCV2022-RIFE")
